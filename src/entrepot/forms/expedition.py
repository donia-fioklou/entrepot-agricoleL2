...
class ExpeditionForm(forms.Form):
    ...
    ligneReception=forms.ModelMultipleChoiceField(queryset=LigneReception.objects.filter(expedier=False), widget = forms.SelectMultiple(attrs={'class':'form-control'}))
    numCont=forms.ModelChoiceField(queryset=Conteneur.objects.all(),widget = forms.Select(attrs={'class':'form-control'}),label='Numéro conteneur')
